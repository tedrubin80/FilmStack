...
class ResendEmailService {
    ...
    async sendEmail(to, subject, textContent, htmlContent = null, from = null) {
        try {
            ...
            const fromEmail = from || process.env.FROM_EMAIL || 'noreply@localhost';
    ...
    async sendWaiverRequestToDirector(data) {
        try {
            ...
            const variables = {
                ...
                platform_contact_email: process.env.PLATFORM_CONTACT_EMAIL || 'support@localhost'
            };
            ...
            return await this.sendEmail(
                ...
                body,
                null,
                `"FestScout" <${process.env.FROM_EMAIL || 'noreply@localhost'}>`
            );
        } catch (error) {
    ...
    async sendNotification(to, subject, message, options = {}) {
        try {
            const fromEmail = options.from || `"FestScout" <${process.env.FROM_EMAIL || 'noreply@localhost'}>`;
...
Have questions? Our support team is here to help at ${process.env.PLATFORM_CONTACT_EMAIL || 'support@localhost'}
    ...
    async testConnection() {
        try {
            ...
            const testEmail = await this.sendEmail(
                process.env.PLATFORM_CONTACT_EMAIL || 'test@localhost',
