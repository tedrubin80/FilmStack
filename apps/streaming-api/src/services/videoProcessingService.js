...
class VideoProcessingService {
    ...
    async processVideo(filmId, jobData) {
        try {
            ...
            // Execute processing script (using execFile to prevent command injection)
            const scriptPath = path.join(this.scriptsPath, 'process_video.sh');

            console.log(`Executing: ${scriptPath} process ${jobData.inboxFile} ${filmId}`);

            // Run processing with progress tracking
            const { stdout, stderr } = await execFileAsync(scriptPath, ['process', jobData.inboxFile, filmId], {
    ...
    /**
     * Update video metadata with local / CDN URLs and file info
     */
    async updateVideoMetadata(filmId, outputFiles) {
        try {
            const uploadRoot = process.env.UPLOAD_PATH || './uploads';
            const cdnBase = (process.env.CDN_HOSTNAME || '').replace(/\/$/, '');
            const baseUrl = cdnBase
                ? `${cdnBase.startsWith('http') ? cdnBase : `https://${cdnBase}`}/videos/${filmId}`
                : `${uploadRoot}/videos/${filmId}`;
    ...
    async getDiskSpace() {
        try {
            const { stdout } = await execFileAsync('df', ['-h', this.processingPath]);
