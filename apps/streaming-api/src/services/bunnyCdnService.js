...
class BunnyCDNService {
    constructor() {
        ...
        this.cdnUrl = (process.env.BUNNY_CDN_URL || process.env.CDN_HOSTNAME || '').replace(/\/$/, '');
        if (!this.cdnUrl && process.env.USE_CDN === 'true') {
            console.warn('USE_CDN=true but BUNNY_CDN_URL / CDN_HOSTNAME is not set');
        }
