    ...
    async processFilm(job) {
        ...
        try {
            ...
            try {
                ...
                console.warn('⚠️ Bunny CDN upload failed, falling back to local storage:', error.message);
                try {
                    cdnUploads = await s3Service.uploadVideoFiles(filmId, outputDir);
                    console.log(`✅ Uploaded to local storage: ${cdnUploads.length} files`);
                } catch (storageError) {
                    console.error('❌ CDN and local storage uploads failed:', storageError.message);
                    // Continue with processed local files
                }
            }
            ...
            ffmpeg.ffprobe(filePath, (err, metadata) => {
                ...
                resolve({
                    ...
                    video: videoStream ? {
                        ...
                        fps: (() => { const parts = String(videoStream.r_frame_rate || '0/1').split('/'); return parts.length === 2 ? parseInt(parts[0]) / parseInt(parts[1]) : parseFloat(parts[0]) || 0; })()
                    } : null,
                    audio: audioStream ? {
