...
class AuthController {
    // Generate JWT access token (short-lived, uses JWT_SECRET)
    ...
    // Generate refresh token (longer-lived, uses JWT_REFRESH_SECRET)
    ...
    static async login(req, res) {
        try {
            ...
            const lockoutTTL = await cache.ttl(lockoutKey);
            if (lockoutTTL > 0) {
            ...
            if (!isValidPassword) {
                ...
                await cache.set(attemptsKey, currentAttempts, 15 * 60); // 15-minute TTL

                // Lock account after 5 failed attempts
                if (currentAttempts >= 5) {
                    await cache.set(lockoutKey, true, 30 * 60); // 30-minute lockout
            ...
            await cache.del(`login_attempts:${normalizedEmail}`);
    ...
    static async refreshToken(req, res) {
        try {
            ...
            // Verify refresh token using dedicated refresh secret
