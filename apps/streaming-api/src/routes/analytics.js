        ...
        const videos = await pool.query(
            `SELECT
             ...
             LEFT JOIN video_analytics va ON v.id = va.video_id
                AND va.date > NOW() - INTERVAL '1 day' * ${safeDays(days)}
    ...
    try {
        ...
        const dailyAnalytics = await pool.query(
            `SELECT date, views_count, unique_viewers, avg_watch_duration,
             ...
             WHERE video_id = $1 AND date > NOW() - INTERVAL '1 day' * ${safeDays(days)}
             ORDER BY date ASC`,
            [videoId]
        );
        ...
        const geoData = await pool.query(
            `SELECT country_code, COUNT(*) as views
             FROM view_sessions
             WHERE video_id = $1 AND started_at > NOW() - INTERVAL '1 day' * ${safeDays(days)}
    ...
    try {
        ...
        const revenue = await pool.query(
            `SELECT date, ad_revenue, tip_revenue, premium_revenue, sponsor_revenue,
             ...
             WHERE channel_id = $1 AND date > NOW() - INTERVAL '1 day' * ${safeDays(days)}
             ORDER BY date DESC`,
            [channelId]
        );
