...
class Video {
    ...
    static async getTrending({ limit = 20, timeframe = '7 days' } = {}) {
        ...
        const safeTimeframe = allowedTimeframes[timeframe] || '7 days';
        ...
        const query = `
            SELECT
                ...
                c.avatar_url as channel_avatar,
                u.username as uploader,
                (SELECT COUNT(*) FROM view_history WHERE video_id = v.id AND created_at > NOW() - $2::interval) as recent_views,
                (SELECT AVG(rating) FROM film_ratings WHERE film_id = v.id) as average_rating
            FROM videos v
            LEFT JOIN channels c ON v.channel_id = c.id
            LEFT JOIN users u ON c.user_id = u.id
            ...
            LIMIT $1
        `;
        const result = await pool.query(query, [limit, safeTimeframe]);
        return result.rows;
    }
