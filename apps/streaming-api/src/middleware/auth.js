...
const MEMORY_CLEANUP_INTERVAL = 60000; // Clean up expired entries every minute
setInterval(() => {
    const now = Date.now();
    for (const [key, data] of memoryRateLimits) {
        if (data.expiresAt < now) memoryRateLimits.delete(key);
    }
}, MEMORY_CLEANUP_INTERVAL).unref();

// Rate limiting middleware with in-memory fallback
const createRateLimit = (windowMs, maxRequests, message) => {
    return async (req, res, next) => {
        ...
        try {
            ...
            const now = Date.now();
            const memData = memoryRateLimits.get(key);
            ...
            if (memData && memData.expiresAt > now) {
                if (memData.count >= maxRequests) {
                    return res.status(429).json({
                        ...
                        error: 'Rate limit exceeded',
                        message: message || 'Too many requests. Please try again later.',
                        retryAfter: Math.ceil((memData.expiresAt - now) / 1000)
                    });
                }
                memData.count++;
            } else {
